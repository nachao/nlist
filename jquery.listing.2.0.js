...
function Listing( opt_options ){
	...
	*/
	...
	this.param_.page_.prev = opt_options.page.prev || null;	// 前一页
	
	this.param_.page_.next = opt_options.page.next || null;	// 后一页
	
	this.param_.page_.current = opt_options.page.current || null;	// 显示当前页
	
	this.param_.page_.total = opt_options.page.total || null;	// 显示总页数
	...
	* @private
	*/
	this.param_.page_.begin = 0;		// 当前开始页数

	this.param_.page_.number = 10;	// 每页显示的数量
...
Listing.prototype.dataPage_ = function( datas ) {
	...
	$(datas).each(function( key, data ){
		...
		if ( key >= begin * number && key < begin * number + number ) {
			reply.push(data);
		}
	});
...
Listing.prototype.dataSet_ = function( param, refresh ) {
	...
	for ( var key in param ) {
		...
		if ( key == 'page' ) {
			for ( var k in param[key] ) {
				that.param_.page_[k] = param[key][k];
			}
			continue;
		}
	...
	if ( param.th && $.type(param.th) == 'object' ) {
		that.param_.th_ = param.th.html();
	}
	if ( param.td && $.type(param.td) == 'object' ) {
		that.param_.td_ = param.td.html();
	}
...
Listing.prototype.dataInitTh_ = function() {
	if ( this.param_.th_ && this.param_.table_ ) {
		// console.log(this.param_.th_);
		this.tableHtml_(this.param_.th_, this);
	}
}
	...
	// 	}
		...
		for ( var key in item ) {
			if ( temp.indexOf( '{{' + key + '}}' ) >= 0 ) {
				...
				templet = temp.replace( reg, item[key] );
				console.log(key + '' + item[key]);
				console.log(templet);
			}
		}

		// console.log(this.param_.td_);
...
Listing.prototype.dataView_ = function( data ) {
	...
	$(data).each(function( key, item ){
		that.tableHtml_( that.dataTemplet_(item), item );
	});
}
...
*
*  @param {Array.<object || array || string>} html = 列表值
*  @param {Array.<object || array || string>} data = 数据
*  @private
*/
Listing.prototype.tableHtml_ = function( html, data ) {

	data = data || {};

	var that = this,
		table = this.param_.table_,
	...
	if ( comm.table ) {
		table.css(comm.table);	// 表样式
	}

	if ( html.indexOf('th') >= 0 ) {		// 如果是表头
		if ( table.find('thead').length == 0 ) {
			table.append('<thead></thead>');
		}
		table = table.find('thead');
	...
	$(html).each(function( i, value ){
		...
		for ( var key in value ) {
			if ( newEl.find(key).length > 0 ) {
				value[key]( newEl.find(key), data );		// 元素事件
			}
			if ( key == 'init' ) {
				value.init( newEl, data );		// 默认TD事件
			}
		}

		if ( value.control && $.isArray(value.control) ) {	// 多个控件
			$(value.control).each(function( i, control ){
				that.tableBinding_( newEl, control, data );
			});
		}

		if ( value.control && $.isPlainObject(value.control) ) {	// 单个控件
			that.tableBinding_( newEl, value.control, data );
		}
	});
...
Listing.prototype.pageGet_ = function() {
	...
	var info = this.dataQuery_(),
		total = info.length,
		page = this.param_.page_,
		number = page.number || 10,
		current = page.begin || 0,
		pages = 0;
...
Listing.prototype.pageDisplay_ = function( info ) {
	...
	if ( this.param_.page_ ) {
		page = this.param_.page_;
	}

	if ( page.current ) {
		page.current.html( info.current );
	}

	if ( page.total ) {
		page.total.html( info.pages );
	}
	...
	if ( page.next ) {
		...
		if ( info.current >= info.pages ) {
			page.next.addClass('disabledButton').attr('disabled', true);
		} else {
			page.next.removeClass('disabledButton').attr('disabled', false);
		}

		page.next.unbind('click').click(function(){
			if ( page.begin + 1 < that.pageGet_().pages ) {
				that.dataSet_({ page: { begin: page.begin + 1 }}, true);
			}
		});
	}
	...
	if ( page.prev ) {
		...
		if ( info.current <= 1 ) {
			page.prev.addClass('disabledButton').attr('disabled', true);
		} else {
			page.prev.removeClass('disabledButton').attr('disabled', false);
		}
